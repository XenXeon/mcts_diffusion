...
Module map (by role; see each file's own docstring for detail):
    sampler / search   mcts_loop (entry point), value_forest, window, stitch
    planner backbones  df_model (Diffusion Forcing), shortcut_df, df_schedule
    evaluators         value_net (V(s)), noise_critic (per-token CG value),
                       grounded (non-learned kitchen checker), value_scale,
                       relabel, coverage
    env / geometry     specs (family constants), maze_oracle (geodesic oracle)
    legacy tree engine tree, node, expansion, rollout (Phase 3/4 ablations)
    instrumentation    instrument, failure_modes
"""
